...
pub async fn emit(nfe: NFe) -> Result<Response, Error> {
    ...
    let cert = Cert::from_pfx("D:/Projetos/dfe/skadao-1234.pfx", "123456")
...
mod tests {
    ...
    #[tokio::test]
    async fn test_from_signed_file() {
        let signed_file = "D:/teste_signed.xml";
        let mut file = File::open(signed_file).expect("Não foi possível abrir o arquivo");
        ...
        // Enviar a NFC-e para a SEFAZ de homologacao de SP
        let url = "https://homologacao.nfce.fazenda.sp.gov.br/ws/NFeAutorizacao4.asmx";
        ...
        match response {
            Ok(response) => {
                ...
                // Se precisar visualizar os cabeçalhos
                ...
                if status.is_success() {
                    ...
                    println!("Resposta Sucesso: {:?}", body);
                } else {
                    println!("Status: {:?}", status);
                    println!("Erro na Requisição: {:?}", body);
                }
            }
            Err(e) => {
                println!("Erro ao enviar a NFC-e: {:?}", e);
            }
        }
    }

    #[tokio::test]
    async fn test_from_unsigned_file() {
        // ************************************* tratamento de xml  */
        let unsigned_file = "D:/teste.xml";
        let mut file = File::open(unsigned_file).expect("Não foi possível abrir o arquivo");
        let mut xml = String::new();

        //let xml_raw_copy = xml.clone();
        file.read_to_string(&mut xml)
            .expect("Não foi possível ler o arquivo");
        // replace <NFe xmlns="http://www.portalfiscal.inf.br/nfe"> por ''
        xml = xml.replace("<NFe xmlns=\"http://www.portalfiscal.inf.br/nfe\">", "");
        // replace </NFe> por ''
        xml = xml.replace("</NFe>", "");
        ...
        // Remove espaços em branco entre '>' e '<'
        let re = Regex::new(r">\s+<").unwrap();
        xml = re.replace_all(&xml, "><").to_string();
        // remove espaços no inicio e final
        xml = xml.trim().to_string();

        //************************************* fim de tratamento de xml  */
        //generate digest value OK
        use sha1::{Digest, Sha1};
        let mut hasher = Sha1::new();
        hasher.update(xml.as_bytes());
        ...
        // generate x509 certificate clean begin and end OK
        let x509_cert = cert::raw_pub_key("D:/Projetos/dfe/skadao-1234.pfx", "123456")
            .await
            .unwrap();
    ...
    + "<SignatureMethod Algorithm=\"http://www.w3.org/2000/09/xmldsig#rsa-sha1\"></SignatureMethod>"
    + "<Reference URI=\"#NFe"
    + "35241254515633000161550010000000011976008519"
    + "\">"
    + "<Transforms>"
    + "<Transform Algorithm=\"http://www.w3.org/2000/09/xmldsig#enveloped-signature\"></Transform>"
    + "<Transform Algorithm=\"http://www.w3.org/TR/2001/REC-xml-c14n-20010315\"></Transform>"
    ...
    + "</SignedInfo>";
        ...
        let signature_base64 =
            openssl_sign(&digest_nodes, "D:/Projetos/dfe/skadao-1234.pfx", "123456")
                .await
                .unwrap();
        let signature_base64 = STANDARD.encode(&signature_base64);
        ...
        //let xml =
        //"<NFe xmlns=\"http://www.portalfiscal.inf.br/nfe\">".to_string() + &xml + "</NFe>";
        ...
        // save xml file to root folder
        use std::io::Write;
        let mut file = File::create("D:/Projetos/dfe/teste_rust.xml")
            .expect("Não foi possível criar o arquivo");
        file.write_all(xml.as_bytes())
            .expect("Não foi possível escrever o arquivo");
    }
