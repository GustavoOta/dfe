...
use crate::nfe::types::manifestacao::{InfEvento, Manifestacao, OperacaoNaoRealizada, Response};
...
pub async fn nfe_confirmacao_operacao(params: Manifestacao) -> Result<Response, Error> {
    enviar_manifestacao(
        &params,
...
pub async fn nfe_ciencia_operacao(params: Manifestacao) -> Result<Response, Error> {
    enviar_manifestacao(
        &params,
...
pub async fn nfe_desconhecimento_operacao(params: Manifestacao) -> Result<Response, Error> {
    enviar_manifestacao(
        &params,
...
pub async fn nfe_operacao_nao_realizada(params: OperacaoNaoRealizada) -> Result<Response, Error> {
    if params.justificativa.trim().is_empty() {
        return Err(Error::msg(
            "A justificativa e obrigatoria para Operacao nao Realizada",
        ));
    }
...
async fn enviar_manifestacao(
    ...
    let digest_value = log_err!(
        crate::nfe::common::cert::DigestValue::sha1(&inf_evento_xml),
    ...
    let signature_base64 = log_err!(
        crate::nfe::common::cert::Sign::xml_string(
            &signed_info,
            &params.cert_path,
            &params.cert_pass
        )
    ...
    let x509_cert = log_err!(
        crate::nfe::common::cert::RawPubKey::get_from_file(&params.cert_path, &params.cert_pass)
            .await,
    ...
    let envelope_xml =
        crate::nfe::common::cleaner::Strings::clear_xml_string(envelope_xml.as_str());
...
fn recepcao_evento_ambiente_nacional(tp_amb: u8) -> Result<&'static str, Error> {
    // tpAmb=1 producao, tpAmb=2 homologacao.
    match tp_amb {
        ...
        _ => Err(Error::msg(
            "tpAmb invalido para RecepcaoEvento no Ambiente Nacional",
        )),
    }
}
